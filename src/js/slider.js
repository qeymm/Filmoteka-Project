...
slideGalleryRef && startWorkSlider();
 ...
 export async function startWorkSlider() {
  try {
    ...
    if (upcomingMovieList && upcomingMovieList.results) {
      renderSlideMovieCards(upcomingMovieList.results);
      slider.mount();
    } else {
      console.warn('No upcoming movie data received from API');
    }
  } catch (error) {
    console.error('Error loading upcoming movies for slider:', error);
  }
}
